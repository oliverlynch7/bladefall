/* WHO IS `by`? — the second door this sub-project has verified a wiring through and the player cannot open
   ...
   **`foeHit` (11275) is `return {name:foeName(e), attack:atk||'a strike'};`** — a DESCRIPTOR, never
   ...
   THREE TRIALS, ONE LAUNCH, one pinned body, `p.guardT` set so the brace is up (the passive lives
   inside `if(p.guardT>0)`):
     1. GAME'S OWN SHAPE — `hurtPlayer(dmg, e.x, e.z, {name:'a grunt', attack:'a melee blow'})`,
        `foeHit`'s output transcribed.
     ...
     3. REAL CONTACT      — the body placed on the hero and the game ticked, so 13500 fires on its
        own with whatever it really passes.
...
(function(){
  ...
  return JSON.stringify({
    dmgIn: DMG,
    gameShape: gameShape, probeDoor: probeDoor, realContact: realContact,
    ...
    toldThePlayerAnyway: (gameShape.bouncedFloaters > 0) || (realContact.bouncedFloaters > 0),
  });
})()
