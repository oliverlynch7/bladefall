...
(function(){
 ...
 function close(service=false){if(!active)return;generation++;stopVoice();const {n,a}=active;active=null;el('hubDialogue')?.remove();document.body.classList.remove('npc-conversation');a.leave(service);if(service)n.open()}
 ...
 function shell(content){
  ...
  root.innerHTML=`<div class="hub-dialogue-panel"><p class="hub-dialogue-label">WAYSTATION</p><h2 id="hubSpeaker">${esc(n.name)}</h2>${content}<footer><button id="hubLeave">Leave conversation <small>Esc</small></button></footer></div>`;
  el('hubLeave').onclick=()=>close();el('hubLeave').focus({preventScroll:true});
 }
 function line(id,lastChoice='',topic='intro'){
  stopVoice();const token=++generation,{n,a}=active,s=state(a,n.id),node=book.nodes[id];if(!node){landing();return}
  s.cursor={id,lastChoice,topic};commit();active.lineId=id;active.started=performance.now();
  shell(`${lastChoice?'<p class="hub-dialogue-reply">You: “'+esc(lastChoice)+'”</p>':''}<p id="hubLine" class="hub-dialogue-line"></p><button id="hubReveal">Show full line</button><p id="hubVoiceStatus" class="hub-dialogue-label"></p><div id="hubChoices" class="hub-dialogue-options"></div>`);
  ...
  function unlock(){if(token!==generation||!active||revealed)return;revealed=true;cancelAnimationFrame(frame);el('hubLine').textContent=node.text;el('hubReveal').hidden=true;
   for(const choice of node.choices){const btn=document.createElement('button');btn.textContent=choice.text;btn.onclick=()=>line(choice.next,choice.text,topic);choices.append(btn)}
   if(!node.choices.length){const btn=document.createElement('button');btn.textContent=topic==='intro'?'Show me.':'Back to services';btn.onclick=()=>{if(topic==='intro'){s.introduced=true;s.cursor=null;commit();close(true)}else{if(topic==='news')s.newsSeen=true;landing()}};choices.append(btn)}
   // A text-skip click cannot become a response click; all choices have a short arming window.
   for(const btn of choices.children)btn.disabled=true;timer=setTimeout(()=>{if(token===generation)for(const btn of choices.children)btn.disabled=false},250);
  }
 ...
 window.BFHubDialogue={open,close,focus,ready,hasNews,known:id=>!!book?.npcs[id],get active(){return active},get speaking(){return speaking}};
})();
