...
for(const zi of [4,5,6,7]){
  const data=await page.evaluate(zi=>{const h=__BF3,Z=h.ZONES[zi];h.meta.classUnlocked.warrior=true;h.meta.trialsDone.warrior=true;h.meta.classId='warrior';h.meta.introSeen=true;h.meta.zoneSeen[Z.id]=true;h.meta.shadeSeen[Z.id]=true;h.enterZone(zi);document.querySelector('#storyskip')?.click();h.G.zone=zi;h.G.zoneTier=Z.tier;h.G.area=-1;h.loadArea(-1);document.querySelector('#storyskip')?.click();const g=h.G,p=g.p,b=g.enemies.find(e=>e.boss);if(!b)return {zone:Z.id,error:'boss missing',stage:g.stageIndex,types:g.enemies.map(e=>e.type)};p.hp=p.maxHp=100000;p.invuln=0;p.x=b.x;p.z=b.z+210;p.y=0;p.vx=p.vy=p.vz=0;b.active=false;b.dropT=.01;const startHp=p.hp,max={projectiles:0,shockwaves:0,trails:0,collapse:0,enemies:0,particles:0},samples=[];for(let i=0;i<900;i++){p.vx=p.vy=p.vz=0;p.x=b.x;p.z=b.z+210;p.y=0;p.yaw=Math.atan2(b.x-p.x,b.z-p.z);if(i===300)b.hp=b.maxHp*.55;if(i===600)b.hp=b.maxHp*.25;h.update(1/60);max.projectiles=Math.max(max.projectiles,g.projectiles?.length||0);max.shockwaves=Math.max(max.shockwaves,g.shockwaves?.length||0);max.trails=Math.max(max.trails,g.trails?.length||0);max.collapse=Math.max(max.collapse,g.collapse?.length||0);max.enemies=Math.max(max.enemies,g.enemies.filter(e=>!e.dead).length);max.particles=Math.max(max.particles,g.particles?.length||0);if(i%60===0)samples.push({t:i/60,hp:Math.round(p.hp),projectiles:g.projectiles?.length||0,shockwaves:g.shockwaves?.length||0,trails:g.trails?.length||0,collapse:g.collapse?.length||0,enemies:g.enemies.filter(e=>!e.dead).length,bossState:{phase:b.phase,slamT:b.slamT,beamT:b.beamT,teleT:b.teleT}});}h.render(g.time);return {zone:Z.id,boss:b.type,startHp,endHp:p.hp,damageTaken:startHp-p.hp,max,samples};},zi);
