...
**The probe was wrong once, and the half that must not move is what said so** — recorded by the run
that wrote it, and the reason the pin at the top of `tick()` is there. Its first run reported the
known-bad at **1.159**: two halves with identical picks, identical minion counts and identical
`dmg` fields dealing 352 and 408. The cause is `G.combo` (10884) — `hitEnemy` multiplies every blow
by `1 + min(0.2, G.combo * 0.004)`, a run-wide counter that climbs as blows land, so it is a
per-trial constant that **drifts upward through a launch**; the control ran first and the known-bad
ninth. Pinned to 0 every frame rather than averaged away: a confound that rises monotonically
through a run does not cancel between halves, it favours whichever half is measured last. Worth
carrying forward, because every dealt-damage probe in this harness is exposed to it. This run's
knownBad read 1, 1, 1 in both launches, which is that pin working.

**Shipped as its own commit**, so Oliver can revert exactly this after playing it. What he is being
asked to judge is a buff to two of the necromancer's three summons; the two cards already promise
it, and the rider that would make it a nerf is still his.
...
**79 probes — 16 synthetic-only, 22 mixed, 21 player-door, 20 neither.** (The census was first written
down as *78 — 21 mixed*, taken before this section's own `desig.probe.js` existed; the instrument is
run over the directory as it stands, so it counted the new probe and the prose did not. Re-measured
after recovery: the extra row IS `desig`, and it lands in **mixed**, which is what a probe that opens
the basic door and the skill door in one launch should be.) `stripNonCode` is IMPORTED
...
**What is NOT swept, and is where a next run should look:** the **22 `mixed`** probes — 21 of them, if
you exclude this section's own `desig`, which is the one row of that verdict already read. A probe that
