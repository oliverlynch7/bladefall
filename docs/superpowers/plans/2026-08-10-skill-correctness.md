...
| 31 | **Q — 68 of the 128 skill cards run another class's function** | this run | `harness/audit-kits.js` + `harness/test/kits.test.js` (11 tests, in the fast stage): 128 cards, 83 distinct handlers, **23 groups shared across classes covering 68 cards**, ratcheted in both directions. The first measurement in this programme aimed at VISION.md priority #2 rather than at skill correctness — nothing in the harness could previously see two classes calling one function. Six cards promise something their shared handler cannot do (the Pirate's Aimed Shot says "rapid… ~2s, softer each" and `deadeye` fires exactly one lance); **all six are Oliver's**, being balance, section K's missing mechanic, or a stale card. Free with it: section A's dead-handler fault is now a STATIC assertion (0 dead, watched) instead of something only a browser cast could catch. The tool's first version reported those nine repaired skills as dead — it read only the first operand of the `A = A \|\| B` repair — and both alias rules are now tests asserting the naive version disagrees |
...
candidate is the stat-snapshot half of Task 3 Step 2, now cheap: against 102 passives that DO have a
...
**A THIRD SEAM WAS OPENED THE SAME DAY AND IT IS THE ONLY ONE POINTED AT PRIORITY #2 — section Q.**
`harness/audit-kits.js` asks a question no tool here had asked: **which classes run the same skill
function under a different name?** 128 cards, 83 distinct handlers, **23 groups shared across classes
covering 68 of the 128 cards** — more than half the game's skill cards are one of 23 functions — now
ratcheted in both directions so a new class wired by aliasing an existing kit cannot land quietly. Six cards promise things their shared handler cannot do — and **all six are Oliver's**,
being balance, section K's missing mechanic, or a stale card, so this seam produces *questions* for
him rather than autopilot rows. Say that plainly rather than leave the next run hunting in it.

**What it does hand the next run is a free static assertion of section A.** A card whose `fx`
resolves to `undefined` — nine shipped that way, each spending its mana and cooldown and doing
nothing — was catchable only by casting the skill in a browser. It is now a millisecond unit test,
so the repair at `index.html:10520-10530` cannot silently come undone.

**And a warning worth carrying into whatever the next tool is.** audit-kits' first version reported
those nine repaired skills as DEAD, because the repair is `A = A || B` and the resolver read only the
first operand. That is the third tool in three passes to fail in the ACCUSING direction — after
`Array.from` splitting the field sweep by code points, and the receiver-restricted reader search. A
false *dead* verdict is the one that gets a non-bug "fixed", so **run any new static tool against the
real game and disbelieve its first red result until the line it names has been read.**
