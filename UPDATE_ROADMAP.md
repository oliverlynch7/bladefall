...
## v1.311.0 — Ian's Blade: the full Blade Shard questline (the emotional payoff, wired at last)
- `STORY.reveal` ("His name was Ian. He was your brother.") was written but never fired. Now it does. Built the full shard-hunt from the story bible (the lighter shop-buy Codex shipped becomes the shortcut/fallback).
- **5 Blade Shards** seeded in the 5 deepest trial chambers (keep→oubliette, frost→glaciervault, ember→magmacore, abyss→reapersgate, palace→reliquary). Finding a trial's rift grants its shard + a `storyToast` of its nameless trail fragment ("…the one before.", "…a name scratched out on the guard.", …, "…'—Ian.'"). Dup-guarded; non-shard trials ignored.
- Name stays **"??? 's Blade"** everywhere (shop, bag, compare) until reforged — `makeWeapon` routes the ians name through `iansBladeName()`.
- **Forge** in the Shop (shows once you hold any shard): reads the recovered fragments, and when all 5 + Level 100 + 250,000g are met, reforges the sword → fires the reveal cutscene → grants + equips the now-named Ian's Blade. The 1,000,000g post-Castle shop row is the shortcut and now also fires the reveal + reveals the name.
- Weapon itself (shipped v1.310): highest DPS in the game, Radiant Execution <25%, +30% vs bosses/elites, Soulfire holy splash (150u, recursion-guarded), Bladeborn's Grace (+15% lifesteal / +12% CDR / +8% speed), full power on all 4 classes (no off-class nerf).
- Verified live: name hidden→revealed, 5 shards granted from the right trials (dup-guarded, non-shard ignored), forge gates (Lv100/250k), reveal cutscene fires (mode=story, all 5 lines), blade equips as "Ian's Blade", normal weapons unaffected, 0 console errors. Exposed the questline fns on `__BF3` for future testing.

## v1.312–1.313.0 — systemic asset grounding + portal keep-clear (the "buried assets / hidden portals" bug class)
...
## v1.317.0 — Outskirts buildings get real collision + secret moved behind a screening post
- **`vsolid(x,z,y0,w,h,d,c,{colH})`** — renders a deco box AND registers a matching collision column, so buildings/crates block the player instead of letting them phase through. Converted the Outskirts farmhouse, pond barn, the three village buildings, and the woodcutter blocks to `vsolid`.
- The tricky bit: several buildings have an intended climb ramp running up through their footprint (line 4020: "Collision stays on the original authored mass"), so naive collision would eject the climber. Fix is general: `resolveObstacles` now never pushes a player out of a `structure` column when they're standing on an elevated walkway (a `plat`/climb-step at their feet). Ground-level phase-through is still blocked. Verified: pushing into the farmhouse at ground level is blocked, and a climber on a step inside the footprint is not ejected.
- **Secret relocated** (per Oliver): the Outskirts rift moved from the Foxglove stump to the barn's BACK loft, hidden behind a stout timber post (`vsolid`, solid). You climb the barn, cross the roof, and round the post to find it. Verified: post blocks the straight center path, player routes around it onto the back ledge and collects the rift; the old Foxglove ledge keeps a chest.
- 0 console errors.

## v1.318.0 — Ruined Keep: building collision + risk/reward secret + survive-to-keep gating
- **Buildings collide**: the Keep siege-camp blocks + broken cart now use `vsolid` (no more phasing through).
- **Secret over the drop**: the Keep rift moved to a tiny ledge stranded past a solid wall, out over the lava/void. Reaching it needs a jump+dash over the wall (175u gap — single jump ~164 falls short into the void; jump+dash ~235 clears the 68u wall and lands it). The old drain-ledge keeps a chest.
- **Respawn on the ledge**: the secret pad is a checkpoint, so once you've reached it a fall drops you right back onto it — you must make the parkour jump BACK to the level to escape (verified: fall from the pad respawns on the pad).
- **Survive-to-keep gating (global, all secrets)**: touching a rift no longer unlocks it — it's now PROVISIONAL (`G.pendingSecret`). It only commits (unlocks the trial portal + grants any Blade Shard) in `nextArea()` — i.e. once you've cleared the level and reached the exit ALIVE. Since dying returns you to the hub, reaching the exit is proof you survived. Die after claiming and the rift slips away. Verified: touch sets pending not sideFound; nextArea commits.
- 0 console errors.

## v1.319.0 — auto-collision sweep (systemic "no more phasing through assets")
- **`autoSolidify()`** (run from finishScape): any chunky, ground-level, free-standing deco mass (building/crate/sarcophagus/block: y0≤12, w≥96, d≥70, h 46–300) that isn't already backed by an authored obstacle gets a matching solid column (`structure:true`, inset by the player radius so the block edge ≈ the visual edge). This is the general fix for "assets you can walk through" — it covers every zone at once, not one asset at a time.
- Verified across all 8 zones: solidified Frost (3 ice blocks) + Castle (10 entrance rubble blocks); Outskirts/Keep already hand-`vsolid`'d (skipped as covered); Ember/Abyss/Palace/Hollow have no free-standing building masses. No auto-col blocks any spawn, exit portal, or climb step (0 conflicts), and the `structure` walkway-guard keeps ramps/climbs that run over a mass from ejecting the player. 0 console errors.

## v1.320.0 — fully remappable key bindings (Settings → Key Bindings)
- Replaced the hardcoded `keymap` with a data-driven binding system: `KEYACTIONS` (14 player actions: move up/down/left/right, jump, attack, dodge, skills 1-4, interact, cycle-loot, toggle-auto), each holding up to 2 physical keys (e.code, so layout-independent). Stored in `meta.keybinds` (persists globally). Reverse lookup `keyAction(code)` drives the keydown/keyup dispatch.
- **Settings → ⌨️ Key Bindings** popup (both pause-menu variants): a row per action with its label + up to two clickable key-caps + a per-action reset (↺). Best-practice UX from research: click a cap → "Press…" listening state, capture the next key; **Esc cancels**; **binding a key that's used elsewhere unbinds it there** (no duplicates / swap-style); per-action **reset to default** and a global **Reset all to defaults**; Done. Menu/UI keys (Esc, Enter) stay fixed and can't be captured.
- Verified live: defaults correct, click→listen→capture rebinds and persists to the global save, conflict-swap works, resets work, and **gameplay input follows the new bindings** (rebound jump to G → Space stops jumping, G jumps, movement intact). 0 console errors.
- Sources for the UX approach: pcgamingwiki Remapping glossary, Rewired ControlMapper docs, gamedeveloper.com settings checklist.

## v1.323.0 — content: THE GAUNTLET (boss rush) + gamepad + accessibility
- **Gamepad support (v1.321):** pollGamepad(dt) each frame — standard mapping (left stick+D-pad move, A jump, X attack, B dodge, Y interact, LB/RB/LT/RT skills 1-4, Start pause, right stick look), auto-detected, Settings toggle.
- **Accessibility (v1.322):** Screen-shake On/Off + Reduce Motion (cuts particles ×0.4 and holds the camera steady) toggles in Settings; persist globally.
- **THE GAUNTLET (v1.323):** a new hub-gated Boss Rush mode. Fight all 7 bosses back-to-back on one life (brute→warden→archer→sorcerer→colossus→king→tyrant), full heal stolen between each, best clear time tracked. New hub gate beside the Arena. Modeled on the Endless framework (own arena builder, tick, win/die screens); the tyrant's death is guarded so it doesn't trip the campaign win. Verified live: full 7-boss clear → win screen + best saved, mid-run death → results, HP bar + banners per boss, 0 console errors.
...
## [Codex | 2026-09-09] Waystation Lantern Court — 1.959.0
Replaced the hub environment with an authored, instanced Blender kit: warm stone courtyard, copper roof service alcoves, lanterns, central Waystone and destination artwork portals. Original interactable coordinates, unlocks, saves and callbacks remain. Removed obstructive asset-pack scenery. Browser QA: all 24 core and hidden approaches reachable and clear; all station prompts checked; Quartermaster dialog and actual Outskirts gate transition work; low quality reports 60 FPS locally with shadows disabled. Kit GLB 277,528 bytes, base static environment 141,308 triangles. Physical mobile performance not measured.
...
## [Codex | 2026-09-11]
Published a gallery-only preview update for Emberdeep, the Abyss, Sunspire Palace and Castle Duskmoor: eight Blender detail/overview images at /3d/art-previews/, explicitly labelled in development. The four gameplay art integrations remain local pending real-browser checks; this gallery commit does not deploy them or change the playable version. Static geometry tests pass across 15 area variants, plus phase-surface exclusion, camera cutaway restoration and vertical shadow tracking. Browser connection unavailable; no frame-rate or live gameplay verification claimed.
## [Codex | 2026-09-11] Final four campaign worlds - v1.964.0-final-worlds-art

Emberdeep (Basalt Furnace), Abyss (Shattered Orbit), Sunspire Palace (Gilded Colonnade), and Castle Duskmoor (Violet Crown) now use custom Blender model kits and instanced scenery across all 15 main/upper/side/boss variants. Original generated collision, routes, interactables and dynamic hazard renderers are retained. Four GLBs total 562,656 bytes; chunk culling, low-quality shadow disable, camera cutaway and altitude-following directional shadows limit the rendering cost. Preview controls no longer capture the pointer. The public art gallery includes detail and full first-area Blender renders for all four worlds.

Validation: real Chromium/WebGL via Playwright CLI on localhost rendered all 15 variants without runtime/shader errors; high/low checks passed and low quality reduced visible geometry with shadows off. Sampled movement, attacks and both cameras across four worlds; all 24 Waystation approaches remain clear/reachable/in range. A save generated by the previous v1.962.0 page resumed in v1.964.0 with gold, weapon and stage intact. Offline real-GLTF tests verified unchanged gameplay inputs across all 15 fixtures, phase surface exclusion, cutaway restoration, finite matrices and altitude-following shadows. Syntax and whitespace checks passed. These are smoke checks, not a complete campaign playthrough or a mobile FPS benchmark. Gallery PNG references checked. Published only to autopilot-merged; main unchanged.
## [Codex | 2026-09-11] Campaign camera polish - v1.965.0-campaign-camera-polish

Reviewed entry and interior views across all eight campaign worlds (including Black Woods). Fixed a reproducible Hollow Pass camera obstruction: at a walkable position near a tall cliff, the rock completely covered the hero and viewport. The renderer now cuts away only the cliff instances between eye and player, restores them after the camera moves, and keeps the standing surface and neighboring instances visible. Geometry, collisions and layouts are unchanged. Original instance matrices remain available for complete Blender exports.

Validation: before/after real-browser screenshots at the same position; all four Hollow variants render without errors in high/low quality; GLTF-backed checks cover occlusion, restoration, same-chunk neighbor visibility, standing surface and unchanged gameplay input. A pre-change v1.964 save preserves gold, weapon and stage, and all 24 hub approaches pass. Syntax and diff checks pass. No mobile FPS claim or full campaign-completion claim.

Oliver clarified the order: campaign graphics first, secondary portal modes when requested, then substantial campaign layout redesign. Recorded in docs/GRAPHICS_AND_LAYOUT_SEQUENCE_2026-09-11.md. The unfinished Endless Dungeon pass is saved locally on codex/endless-dungeon-art (5a5b424), not deployed. This ship contains only the campaign camera fix and sequencing note; autopilot-merged only.

## [Codex | 2026-09-12] Enemy art and animation — v1.966.0-enemy-art

Replaced the entire 40-type enemy roster plus the Marble Colossus appearance with 41 original low-poly Blender models, each with ten bones and Idle/Move/Windup/Attack/Hit/Death clips. Biome palettes and silhouettes match the new campaign environments. Maximum 848 triangles per appearance; 4.81 MB for all models, loaded on demand. Stable per-enemy skeletons, bounded corpse presentation and pooling, shared geometry, hit/fade feedback and preserved role/shield telegraphs. Gameplay timing, dimensions, progression and saves are unchanged.

Validated all models, all campaign biomes/bosses in real Chrome/WebGL at high/low quality, real hits/deaths and pool reuse, missing-asset fallback, previous-version save loading and the mobile viewer. Settled desktop samples reached 60 FPS; no claim of a phone hardware benchmark. Blender stills and interactive animation previews: /3d/art-previews/enemies/. Source and evidence: tools/art/ and docs/art-validation/. Deploy target: autopilot-merged only.
...
## [Codex | 2026-09-23] Final King and restoration — 2.022.0
