...
**Where the next run should look, in order.** `bd_feet` was taken the same run, as pass 32 — so the
next is **`bd_fast`**, the same call a third time: "a parry refunds the time your attack would have
taken" needs the catch at `hurtPlayer` 11536 to zero `p.atkCd`, which `playerAttack` sets and which
`w_swift` already zeroes three lines from where it is set. Nothing has to be chosen. `w_juggernaut`
