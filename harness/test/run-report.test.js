...
/* THE DEFAULT PATH, WHICH IS THE ONLY ONE THE CLI USES — and which was wrong: it looked for
   `<repo>/baseline.json` rather than `harness/baseline.json`, so every report silently lost its
   "N known failures still tracked" line. The test above cannot catch that, because a missing file
   legitimately returns null. This one reads the file a SECOND way and demands the two agree, which
   is the only version of this assertion that can fail. */
test('baselineCount with no argument finds the real baseline', () => {
  const f = join(ROOT, 'harness', 'baseline.json');
  const want = JSON.parse(readFileSync(f, 'utf8')).known.length;
  assert.strictEqual(baselineCount(), want);
  assert.ok(want >= 0);
  /* The old default, kept as the failing case: there is no baseline.json at the repo root, so the
     bug's return value was null and the report line vanished. */
  assert.strictEqual(baselineCount(join(ROOT, 'baseline.json')), null);
});
