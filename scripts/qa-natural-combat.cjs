async page=>{
 ...
 for(let z=0;z<8;z++)for(const area of [0,1,-1]){
  b.enterZone(z);if(area===1)b.nextArea();if(area===-1){b.nextArea();b.nextArea();}const G=b.G,p=G.p;
  ...
  let e,spot;for(const enemy of candidates){if(!enemy)continue;for(const distance of [100,70,40]){for(let a=0;a<8;a++){const q={...enemy,x:enemy.x+Math.sin(a*Math.PI/4)*distance,z:enemy.z+Math.cos(a*Math.PI/4)*distance,r:p.r},floor=b.enemySupport(q);if(!Number.isFinite(floor)||Math.abs(floor-enemy.y)>45)continue;const before=[q.x,q.z];b.resolveObstacles(q,q.r,null,0);if(Math.hypot(q.x-before[0],q.z-before[1])>2)continue;spot={x:q.x,z:q.z,y:floor};e=enemy;break;}if(e)break;}if(e)break;}
  ...
  results.push({z,area,type:e.type,active:e.active,movement:Math.round(maxMove),attack,projectile,states:[...states],damage:Math.round(start.hp-p.hp),mode:b.mode,player:[p.x,p.y,p.z]});
 }
 return results;});if(errors.length)throw Error(JSON.stringify(errors));for(const r of result){if(r.error||!r.active||r.damage<=0||(!r.attack&&!r.projectile&&!r.states.includes('strike'))||(r.area>=0&&r.movement<10))throw Error('Combat failed: '+JSON.stringify(r));}return {result,errors};
}
