...
| 35 | **Q — berserker Rage doubled your damage and never stopped you healing** | this run | `harness/probes/rage.probe.js`, THREE halves in one launch with TWO trials each (at 40% health and at 15%, because "below a quarter" is half the sentence) against TWO independent heal paths — the Arena's own heal orb collected by the game's `arenaPickupTick`, and lifesteal off a real one-frame `input.attack` press whose 8% is Undying Rage's own: control and known-bad healed in all eight of their readings, the passive half healed at 40% (+191 orb, +14 lifesteal) and healed **nothing at all** at 15% — 72 HP in and 72 HP out of a 477 HP hero. `okAgainstInert false`. **The first row whose missing half is the DRAWBACK**, so shipping it makes the class weaker on purpose; pass 20's rule is what licenses that. **The `healPlayer()` door this plan predicted was needed was deliberately NOT built** — ~20 heal sites across every class, and every one a door missed would be a silent hole in a drawback, so the lock is one end-of-frame comparison in `update()` that catches every heal path including ones added later. The bar is the health you HAD, not the health you end up with, or the 40%-of-max orb would have escaped it by carrying the hero back over the line. Three bench faults first, each of which had the probe measuring nothing while looking healthy: the Arena's orbs are an opt-in variant (`ARENA_LOADOUT.powerups`) and six trials read `orbTaken:false`; a blade's REACH is not its aim range, so a dummy at 120 units was aimed at and never touched; and **the hero is not drawn on alternating i-frames**, so the first attempt at the picture photographed an empty arena with a correct HUD. Photographed at `_shot/out/rage-blocked.png`. Berserker suite 4 pass / 1 fail either side, the fail being the baselined Charge |
...
**`bsk_rage` WAS TAKEN — pass 35 — AND THE REFACTOR THE PARAGRAPH ABOVE PREDICTED WAS THE WRONG
ANSWER.** The count was right (~20 `p.hp=Math.min(effMaxHp(p),p.hp+X)` sites, across lifesteal,
pickups, class riders, death saves and the Abyss breather) and the conclusion drawn from it was not.
A `healPlayer()` door is a refactor across every class, it can only ever cover the sites that existed
the day it was written, and — the part that matters for a card like this — **every site a door misses
is a silent hole in a DRAWBACK**, which is the dead passive again, reached more expensively. The lock
is instead one comparison at the end of `update()`: the health at the start of the frame against the
health at the end. One place, every heal path in the game including ones added later, nothing
per-site to get wrong. It costs one thing and the code says so: a blocked heal is undone rather than
refused, so the fix prints its own floater to tell the player why the green number did not stick.
**The general lesson, and it is not specific to healing: "there is no central door" is a reason to
find the one place that observes the EFFECT, not a reason to build a door.**

**So the next two, cheapest first, are `w_juggernaut`** (still gated on its one-sentence English
question, still Oliver's) **and then `pal_heal`** — whose amount, this document's own entry says,
must be argued from the file rather than chosen, since the nearest candidate (the 4%-of-max that
`m_ward`/`war_shield`/`st_ward` grant per cast) is a SHIELD unit and not a heal unit. That argument
is the row's real blocker and it is one Oliver can settle in a sentence.
