async page=>{
 ...
 try{
  ...
  const field=async key=>{await guest.evaluate(key=>{const b=__BF3,o=b.G.storyObjects.find(o=>o.key===key);Object.assign(b.G.p,{x:o.x,z:o.z,y:o.y});b.MP.hostConn.send({t:'pos',p:b.MP.selfState()});b.briarRequest('world',{key});},key);await flush()};
  await field('home.orchard.wheel');ok('orchard walkway deploys on both peers',await page.evaluate(()=>__BF3.G.obstacles.filter(o=>o.orchardBridge).length===1)&&await guest.evaluate(()=>__BF3.G.obstacles.filter(o=>o.orchardBridge).length===1));
  await field('home.farm.supplies');await field('home.orchard.cache');ok('both field discoveries reward both peers',await page.evaluate(()=>__BF3.G.storyClaims['home.farm.supplies']&&__BF3.G.storyClaims['home.orchard.cache'])&&await guest.evaluate(()=>__BF3.G.storyClaims['home.farm.supplies']&&__BF3.G.storyClaims['home.orchard.cache']));
