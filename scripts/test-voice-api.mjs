...
let source=await fs.readFile('functions/voice-api/[[path]].js','utf8');source=source.replace(/import briar[^;]+;\s*import hub[^;]+;\s*import hall[^;]+;\s*import cliffs[^;]+;\s*import canyon[^;]+;\s*const book=[^;]+;/,'const book='+JSON.stringify(book)+';');
