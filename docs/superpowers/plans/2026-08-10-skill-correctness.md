...
**AND THE FOURTH SWEEP WAS THE ONE THIS PLAN HAD BEEN NAMING SINCE PASS 43, 2026-08-13 — pass 54,
section Z, and it came back EMPTY.** Section Y being spent left exactly one lead written down
anywhere: *"every probe in `harness/probes/` that drives an effect through `hitEnemy`, `hurtPlayer` or
a direct field assignment rather than through the button a player presses is open to this, and none of
them has been re-read with that question in mind. That is the next sweep, and it needs no game change
to run."* It has now been run, with an instrument and a both-directions ratchet
(`harness/audit-probes.js`), and **all sixteen synthetic-only probes are already owned by section U,
by section W, or are on a melee class where the probe's door and the player's door are the same line.
No new row.** Recorded as a spent lead so nobody re-derives it.

**WHERE THE NEXT RUN SHOULD LOOK, 2026-08-13, after four sweeps in two days.** In order, cheapest
first, and each is scoped by a table that already exists rather than by ambition:

- **the 21 `mixed` probes** (section Z's own closing paragraph). The instrument can say a player door
  is used; it cannot say the BAR rides on it. Reading them needs no launch and no game change, and it
  is the same question section Z just answered for the easy sixteen. **This is the direct continuation
  of pass 54 and the only lead that is already scoped.**
- **section U/U2/U3's switch-on**, which is one sentence from Oliver and then a one-line commit plus a
  re-measure with two committed probes. It is worth putting to him again precisely because section Z
  has now re-confirmed, mechanically, that five of this plan's own passes are unreachable without it.
- **section Y's Hunter's Mark**, the row section Y says to put to him first — half that card
  (+10% crit) has no implementation anywhere, so it is unbuilt rather than merely mis-numbered.

And the standing rule this document keeps having to re-learn, now four for four: **a row is only as
...
read in exactly four places in the game. Two of the four were fixed by pass 47 (section W); of the
other two, the ranger's is pass 55 and the paladin's Oath is Oliver's. **The generalisation is the
part to carry: once a probe's bar is found to ride on an argument, sweep every reader of that
argument** — one grep turned one row into the complete population of that bug class, and it is closed
rather than sampled.

**WHERE THE NEXT RUN SHOULD LOOK, after pass 55.** The `mixed` lead is spent and its write-up says so.
What is left, in order:
- **section U/U2/U3's switch-on** — one sentence from Oliver, then a one-line commit and a re-measure
  with two committed probes. Section Z re-confirmed mechanically that five of this plan's own passes
  are unreachable without it; section AA has now added a sixth reader-shaped question beside it.
- **section AA's paladin Oath** — one sentence from Oliver, then `attackerOf(by)` and a comparison.
- **section Y's Hunter's Mark**, the row section Y says to put to him first: half that card (+10%
  crit) has no implementation anywhere, so it is unbuilt rather than mis-numbered.
All three are Oliver's, so **the next autopilot pass is again a sweep and it is not yet named.** The
one shape this document has never run: `hurtPlayer`'s `by` was swept because a probe pointed at it —
every other ARGUMENT the game passes between subsystems (`hitEnemy`'s `src`, `c2OnKill`'s `e`,
`killEnemy`'s body) has the same failure mode and none has been censused.
...
**PASS 53 WAS TAKEN — 2026-08-13 — and the interesting part is not the edit, it is that the pass had
to be RECOVERED FROM A STASH TWICE OVER before it landed.** The 08:04 run made the edit, measured it
either side and wrote it up; `run-all.js` then went red on two `mp:/ping target:` rows that no warrior
damage literal can reach, and `autopilot.ps1` stashed the whole tree (`stash@{0}`, *autopilot gate-red
2026-08-13 08:27*). The 08:44 run recovered it, **re-measured rather than trusted it** — the probe's
own absolute bar, `chargeIsOnItsCard true` / `chargeIsStillTheOldNumber false` /
`chargeOverStomp 1.000` with `whirlOverStomp` at 1.371 beside it — and shipped it. The gate that ate
it was diagnosed and fixed in the same run: harness-hardening **Task 7**, a flap in how
`pingtgt.probe.js` chose its subject.

**Read the write-up in `docs/SKILL_TRIAGE.md` for why that after-reading is a BAR and not a delta**,
and why this run deliberately does not claim a before/after pair it did not take. The 08:04 run's
pair (0.933 → 1.000, Charge's dealt 84 → 90) is preserved below as that run's evidence.

The set-up note, kept because it is the cheapest hand-off this plan has recorded: the instrument
committed, the before-reading printed, the exact edit named by file and line, the alias hazard already
settled, the follow-up commands listed. The row is `w_charge`, section Y's only takeable one. The
change is a single character in a single literal: `public/3d/index.html:9914`, `wcharge`'s `*1.5*` →
`*1.6*`, which is the number on the card at 2039. The alias hazard is settled in section Y — `wcharge`
has one definition, and `bsk_charge` is redefined at 19711 as Headlong, so the berserker is NOT
affected. The instrument is committed as `harness/probes/charge.probe.js` and its BEFORE reading was
taken on unmodified game code by the run that set the pass up:
...
| pass | row | commit | how it was proven |
|---|---|---|---|
| 55 | **AA — the ranger's Bounty Hunter took its −8% off a `by` that is never a body** | this run | The direct continuation of pass 54 and the lead this document names as *"the only lead that is already scoped"*: reading the **21 `mixed`** probes for whether the BAR rides on the synthetic door. Nineteen are settled without a launch by two of section Z's own rules (a melee class's `hitEnemy(…,p,…)` IS `resolveSwing`'s line; a ranged class's is section U, and all five are already in section U's table) plus seven individual reasons worth keeping — `legion` measures MINION damage, `swagger` measures distance, `chrhaste` passes the `null` the game really does pass at 12997, `skyarmor`'s bar is an invulnerability window that never reads `by`. **The twentieth is the row.** `bounty.probe.js` takes every hit TWICE — once with the enemy body as `by`, the positive control that says the wiring exists, and once through `__BF3.foeHit(foe,…)`, the game's own function rather than a copy of its shape — and reports `ok` beside `reachableInPlay` without folding them, because **`ok:true, reachableInPlay:false` IS the row.** Measured, one launch each side: before `ok true / reachableInPlay false`, marked 619 against plain 619 = **1.000** on the game's door; after `true / true`, 556 against 604 = **0.921**, the card's own −8%. The control half reads **604/604 on both doors in both runs**, and `okAgainstInert` / `reachableAgainstInert` are false in both — the green light watched to go red. The fix is `attackerOf(by)`, the helper section W added for this exact shape, already used by the two reflect readers at 11625/11724; nothing invented. **Following the row up produced a COMPLETE CENSUS of the bug class rather than one instance:** `by` is read in exactly four places, two fixed by pass 47 and two never revisited — the fourth is the paladin's Oath at 11647, whose `by !== G.p._oath` is therefore always true, so an oathed paladin takes a third less from *everything including its Sworn target* and the mechanic's whole drawback does not exist. **That one is Oliver's** and is written up as such: the Oath is on no card (section R), so making it work is a nerf with no card number behind it. `docs/SKILL_TRIAGE.md` section AA. Photographed at `_shot/out/aa-bounty-after.png` |
| 54 | **Z — the probes themselves, read against the door a player presses** | this run | A SWEEP, not a row, and the one pass 43 named ("that is the next sweep, and it needs no game change to run") — never taken until now, and the only lead left after section Y was spent. `harness/audit-probes.js` + `harness/test/probes.test.js` (13 tests, fast stage) sort all **79 probes by the doors they open**: 16 synthetic-only, 22 mixed, 21 player-door, 20 neither (written down as 78/21 before this pass's own `desig.probe.js` was counted — the instrument reads the directory as it stands and the prose did not; re-measured on recovery, the extra row IS `desig` and it lands in **mixed**, which is what a probe that opens the basic door and the skill door in one launch should be). **The result is a negative one and it is recorded as one: no new row.** All sixteen synthetic-only probes are already owned by section U (five, all ranged classes reaching `CLASS_BASIC` through the melee door), by section W (one), or are on a MELEE class where `hitEnemy(…,p,…)` is the same line `resolveSwing` calls — so this lead is spent. **The launch it cost bought the claim all sixteen verdicts rest on, and that claim was READ WRONG first.** `hitEnemy`'s dispatch is `src===G.p && !G._desig` with nothing in it saying "basic attack", and skill handlers call `hitEnemy(e,dmg,p,…)` at thirty-odd sites — so the source says every direct-hit SKILL runs the hook, which for a mage would be 6 mana and ×1.35 **per body struck**. `harness/probes/desig.probe.js` measured it instead: basic door **6 mana for one body and 24 for four** (scales), skill door **22 for one body and 22 for four** (flat), `okIfSkillsFiredTheHookToo false` beside `ok true`, `playTicks 2416 of 2416`. `useSkill` sets `G._desig=true` around the whole cast (10649/10659), so the hook is correctly basic-attack-only. **The probe was wrong twice first and both times produced plausible numbers**: `p.mana=100000` does not survive a frame (the pool is clamped; every trial read ~99933 "spent"), and the mage's rank-10 rider refunds half the cost on every third cast, so `_mageCasts` is pinned before each measured cast. The both-directions ratchet **fired on its own author** — the `KNOWN_SYNTHETIC` list was filled in by eye before measuring and the reverse check named the seven entries that were wrong. `docs/SKILL_TRIAGE.md` section Z |
| 53 | **Y — warrior Charge dealt 1.5x off a card that says 1.6x** | this run | Section Y's one takeable row, taken in one edit and one launch. Nothing was chosen: 1.6 is the card's own number at 2039 and `wcharge` has exactly one damage literal. **The probe is a BAR, not a delta, and that is what let a recovered edit be re-verified without a before:** `harness/probes/charge.probe.js` reads Charge against SHOCKWAVE STOMP cast at the same body with the same weapon in the same launch, so the claim is the absolute relation `chargeOverStomp == 1.000` — reported as `chargeIsOnItsCard true` beside `chargeIsStillTheOldNumber false`, two fields that cannot both be true. Measured: **1.000** (Charge 105, Stomp 105), `clean true`, `playTicks 160 of 160`, `sameRig true`. **`whirlOverStomp` is the half that must not move** — Whirlwind is the b-side of Charge's own rank, the closest thing in the game this edit could splash onto — and it sits at 1.371 rather than moving with it. **Absolutes are exactly what is not compared:** this launch's control cell reads 105 where the 08:04 run's read 90, a different starter roll, the same reason two launches of this probe once read the identical control at 4.2973 and 5.0278 (pass 49). **The berserker is untouched and it was checked rather than assumed** — `bsk_charge` aliases this body at 10374 and is REDEFINED at 19711 as Headlong, and the last assignment wins; the note is in the game file at the edit site. **Recovered from `stash@{0}`**, a red gate's stash of a previous run's finished work, where the red had nothing to do with it — the flap that caused it is fixed in the same run as harness-hardening Task 7. **Section Y is now spent**: its other twelve rows are all the code being MORE generous than the card, so each is a nerf and stays Oliver's |
