const returns=JSON.parse(await fs.readFile('public/3d/story/return-visits.json','utf8'));
const finale=JSON.parse(await fs.readFile('public/3d/story/final-king.json','utf8'));
...
const book={npcs:{...briar.npcs,...hub.npcs,...hall.npcs,...cliffs.npcs,...canyon.npcs,...keep.npcs,...prison.npcs,...peaks.npcs,...caves.npcs,...iron.npcs,...furnace.npcs,...shore.npcs,...thunder.npcs,...court.npcs,...library.npcs,...orb.npcs,...castle.npcs,...ascent.npcs,...finale.npcs,...returns.npcs},nodes:{...briar.nodes,...hub.nodes,...hall.nodes,...cliffs.nodes,...canyon.nodes,...keep.nodes,...prison.nodes,...peaks.nodes,...caves.nodes,...iron.nodes,...furnace.nodes,...shore.nodes,...thunder.nodes,...court.nodes,...library.nodes,...orb.nodes,...castle.nodes,...ascent.nodes,...finale.nodes,...returns.nodes}};
let source=await fs.readFile('functions/voice-api/[[path]].js','utf8');source=source.replace(/import returns[^;]+;\s*/g,'');source=source.replace(/import finale[^;]+;\s*import ascent[^;]+;\s*import castle[^;]+;\s*import orb[^;]+;\s*import library[^;]+;\s*import court[^;]+;\s*import thunder[^;]+;\s*import briar[^;]+;\s*import hub[^;]+;\s*import hall[^;]+;\s*import cliffs[^;]+;\s*import canyon[^;]+;\s*import keep[^;]+;\s*import prison[^;]+;\s*import peaks[^;]+;\s*import caves[^;]+;\s*import iron[^;]+;\s*import furnace[^;]+;\s*import shore[^;]+;\s*const book=[^;]+;/,'const book='+JSON.stringify(book)+';');
