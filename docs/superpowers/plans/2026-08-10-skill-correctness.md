...
| 14 | **E — chronomancer Echo was never read** | this run | `harness/probes/chrecho.probe.js`, THREE halves in one launch with THREE timed windows each: the control's Singularity dealt 388 on the cast and nothing at all afterwards; the passive half dealt 388 and then **388 again with no input**, ratio exactly 1, and nothing in the tail. Chronomancer suite 4 pass / 0 fail after; passives 87 wired → 88 |
