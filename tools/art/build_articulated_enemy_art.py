...
OUT=ROOT/'public/3d/enemy-assets/articulated'; OUT.mkdir(exist_ok=True)
...
parts=[]; palette=[]; articulated=False
def rgba(h):
 c=[int(h[i:i+2],16)/255 for i in (0,2,4)]
 return tuple(((v+.055)/1.055)**2.4 if v>.04045 else v/12.92 for v in c)+(1,)
def tag(o,b,col):
 if articulated:
  if b.startswith('upper'):b='arm'+b[-1]
  elif b in {'armL','armR'}:b='forearm'+b[-1]
...
def humanoid(name):
 ...
 for s,b,leg in [(-1,'armL','legL'),(1,'armR','legR')]:
  x=s*(w+.035); elbow=(s*(w+.06),-.0175,.53); knee=(s*.105,0,.245)
  upper='upper'+b[-1] if articulated else b
  ico((x,0,.705),(.12 if heavy else .08,.10,.09),2 if heavy else 1,upper)
  if articulated:
   cone((x,0,.68),elbow,.065 if heavy else .045,.06,1,upper)
   ico(elbow,(.068,.068,.068),0,upper)
   cone(elbow,(s*(w+.085),-.035,.38),.06,.072 if heavy else .044,1,b)
   cone((s*.10,0,.39),knee,.072 if heavy else .046,.05,1,leg)
   ico(knee,(.055,.055,.055),2,leg)
   cone(knee,(s*.11,0,.095),.05,.05,1,'shin'+leg[-1])
  else:
  ...
  box((s*.11,-.045,.065),(.135,.205,.10),0,'shin'+leg[-1] if articulated else leg)
 if robe:
 ...
 if name in {'sentinel','siegeknight','warden'}:
  ...
  for s,b in [(-1,'armL'),(1,'armR')]:
   ico((s*.32,0,.72),(.17,.14,.15),1,'upper'+b[-1] if articulated else b)
  ...
  if name=='tyrant':
   for s,b in [(-1,'armL'),(1,'armR')]:
    for i in range(3):spike((s*(.23+i*.045),.015,.74),(s*(.30+i*.07),.02,.97-i*.06),.065,2,'upper'+b[-1] if articulated else b)
  ...
  if name=='frostling':
   for s,b in [(-1,'armL'),(1,'armR')]:spike((s*.23,0,.72),(s*.31,.02,.93),.08,2,'upper'+b[-1] if articulated else b)
...
def rig_and_export(name):
 ...
 if articulated:
  w=.255 if name in HEAVY else .18
...
def build(name):
 ...
 articulated=name not in QUADS|FLOAT|{'bones','slime','slimelet','dummy','mimic','embertotem'}
...
names=args or [n for n in ROSTER if n not in QUADS|FLOAT|{'bones','slime','slimelet','dummy','mimic','embertotem'}]
