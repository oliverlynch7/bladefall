...
const HERE = import.meta.dirname;                        // harness/
const ROOT = join(HERE, '..');                           // the repo, which is what git wants as cwd
...
/* How many known failures the ratchet is still carrying. Missing file is not zero — an absent
   ...
   THE DEFAULT PATH WAS WRONG AND THE TEST COULD NOT SEE IT: it read `<repo>/baseline.json`, and the
   file is `harness/baseline.json`. Missing-file is a legitimate state that returns null, so the CLI
   quietly dropped the "N known failures still tracked" line on every run and nothing said so — the
   only assertion was that a made-up path returns null, which the bug satisfies. Fixed here, and the
   test now also asserts the DEFAULT finds the real file with the count read a second way. */
export function baselineCount(file){
  const f = file || join(HERE, 'baseline.json');
